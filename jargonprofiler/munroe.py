...
def munroe_score(text, exclusions='', verbose=True):
    '''Calculate the 'Munroe score' for a text.

    Takes raw text, tokenises and stems it, and compares the stems to the set
    of the stemmed 1000 most common words in American English.
    Returns the percentage of words that were in the list of common words,
    and a dictionary mapping words to their categories (common, proper noun,
    etc.).

    E.g. if output is {'score': 0.61, 'tagged_words': {}}, 61% of words were
    in the list of the 1000 most common.
    '''
    # Process exclusions
    if exclusions != '':
    ...
    # Find all words - alphanumeric strings not separated by punctuation
    ...
    # Mark proper nouns and non-alphabetic words in the tag list.
    tokens = []
    for i, word in enumerate(words):
        # Check if the word is a proper noun.
        # If it is, mark it and put an empty string in the list of tokens
        if word in proper_nouns:
            ...
            # If there is more than one token, it means the word was broken
            # by a number, In this case, ignore it.
            # If there are no tokens, it means that there were no alphabetic
            # characters in the token. Ignore it.
            if len(token) != 1:
                tokens.append('')
    ...
    if verbose:
        ...
        print('Of these, ' + str(munroe) +
              ' are in the most common 1000 words!')
